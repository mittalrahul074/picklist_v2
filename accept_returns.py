...
def _load_returns_data() -> pd.DataFrame:
    """
    Load returns data from the database with proper session state management.
    ...
    if SESSION_KEY_RETURN_DF not in st.session_state:
        st.session_state[SESSION_KEY_RETURN_DF] = get_returns_from_db(RETURN_STATUS)
...
# -------------------------------------------------------------------
def render_accept_returns_panel() -> None:
    """
    Render the main accept returns panel interface.
    
    This function orchestrates the entire accept returns workflow:
    ...
    # Load and filter returns data
    returns_df = _load_returns_data()
    filtered_df = utils.get_party_filter_df(returns_df, party_filter)
